...
- **깜빡이**: 낱말·어절·문장 노출 (글자수 자동/고정/무제한, 배속 슬라이더 + 여유 프리셋 4단,
  셔플·반복·구간, 카운트다운, 전체화면, 글자 크기 배율)
- **연습 제어**: ⏸ 일시정지(모바일 포함), 일시정지 중 진행바 드래그·◀▶·화살표로 위치 이동,
  중간 종료·이탈 시 자동 저장 → [이어서 시작] (타이핑 입력까지 복원)
- **무제한 모드**: 타이머 없이 맞게 치면 자동으로 다음 (IME 안전 — 띄어쓰기 확정 후 판정)
- **타이핑 채점**: 지연 허용 정합(반 박자 늦게 쳐도 정확), 음절/자모/타수 단위,
  띄어쓰기·문장부호 무시 옵션, **공인시험 기준 프로필**(음절 감점·첨가 감점),
  실시간 정확도·타수(토글), 오답만 재연습, 입력 전문 보기·복사
...
- **기본 단어장 10종**(`seed/`): 낱말→겹받침→사자성어→구→속담→문장 난이도 사다리, 각 100문제
